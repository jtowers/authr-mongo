    ...
    beforeEach(function (done) {
     authr_config = {
        user: {
          ...
          password_reset_token: 'account.reset_token',
          password_reset_token_expiration: 'account.token_expires',
      ...
      it('should be able to find a password reset token', function (done) {
        adapter.getUserByUsername('test@test.com', function (err, user) {
          ...
          adapter.generateToken(20, function(err, token){
            ...
            adapter.findResetToken(user.account.reset_token, function (err, user) {
              done();
            });
