...
Adapter.prototype.unlockUserAccount = function (callback) {
  ...
  this.db.collection(this.config.db.collection).update(query, this.user, function (err, docs) {
    if(err) {
      throw err;
    }
    if(!docs) {
...
Adapter.prototype.verifyEmailAddress = function (callback) {
  ...
  this.db.collection(this.config.db.collection).update(find_query, this.user, function (err, user) {
    if(err) {
      throw err;
    }
