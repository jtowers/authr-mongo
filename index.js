...
Adapter.prototype.isValueTaken = function (object, path, callback) {
    ...
    this.db.collection(this.config.db.collection).findOne(query, function (err, doc) {
        if(err) {
            callback(err, null);
        }
...
Adapter.prototype.resetPassword = function (user, callback) {
    ...
    this.db.collection(this.config.db.collection).update(query, user, function (err, doc) {
        if(err) {
            throw err;
        }
        if(!doc) {
...
Adapter.prototype.findVerificationToken = function (token, callback) {
    ...
    this.db.collection(this.config.db.collection).findOne(query, function (err, user) {
        if(err) {
            return callback(err);
        }
        if(!user) {
...
Adapter.prototype.verifyEmailAddress = function (user, callback) {
    ...
    this.db.collection(this.config.db.collection).update(find_query, user, function (err, doc) {
        if(doc) {
            return callback(null, user);
        } else {
...
Adapter.prototype.deleteAccount = function (user, callback) {
    ...
    var username = this.getVal(user, this.config.user.username);
    var query = this.buildSimpleQuery(this.config.user.username, username);
    this.db.collection(this.config.db.collection).remove(query, {}, function (err, docs) {
        if(err) throw Err;
        if(!docs) {
...
Adapter.prototype.unlockUserAccount = function (user, callback) {
    ...
    var query = this.buildSimpleQuery(this.config.user.username, this.getVal(user, this.config.user.username));
    this.db.collection(this.config.db.collection).update(query, user, function (err, docs) {
        if(err) {
            throw err;
        }
        if(!docs) {
...
Adapter.prototype.lockUserAccount = function (user, callback) {
    ...
    query = this.buildSimpleQuery(this.config.user.username, this.getVal(user, this.config.user.username));
    this.db.collection(this.config.db.collection).update(query, this.user, function (err, doc) {
        if(err) {
            throw err;
        }
...
Adapter.prototype.resetCollection = function (callback) {
    this.db.collection(this.config.db.collection).remove({}, {
        multi: true
    }, function (err) {
        if(err) throw err;
        callback(err);
